...
{{< cards col="3">}}
  {{< card link="/addons" title="Addons" image="https://res.cloudinary.com/dhmmkvcpy/image/upload/v1780572918/schedule_vpqtah.png" subtitle="Addons vi använder i gillet">}}
  {{< card link="/bank" title="Gillsbank" image="https://res.cloudinary.com/dhmmkvcpy/image/upload/v1780573002/ledger_de61so.png" subtitle="asd">}}
...
#### Senaste inlägget
{{< cards col="5">}}
  {{< recent-content count="1" >}}
  {{< card link="/artiklar" title="Äldre inlägg" subtitle="Historik" >}}
{{< /cards>}}
  ...
  if (!customElements.get('guild-online-widget')) {
    class GuildOnlineWidget extends HTMLElement {
      ...
      async runCycle() {
        ...
        let nextDelay = 60000; // 60-second baseline fallback
        ...
        try {
          ...
          if (lastModifiedHeader) {
            ...
            // Format time string safely (HH:MM:SS)
            const dateObj = new Date(fileModifiedTime);
            formattedTime = dateObj.toTimeString().split(' ')[0];

            // 10 minutes checking boundary (10 * 60 * 1000 = 600000ms)
            if (ageInMs >= 600000) {
              isStale = true;
              nextDelay = 10000; // Fast-poll every 10s to recover instantly when backend wakes up
